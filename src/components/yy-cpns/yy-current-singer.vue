<template>
    <v-card>
        ...
        <v-toolbar dark color="primary" dense class="rounded-0">
            <v-icon class="mx-4" large>mdi-account-music</v-icon>

            <v-toolbar-title>{{
                $store.state.currentSinger.singerName
            }}</v-toolbar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
                <v-btn dark text @click="$emit('input', false)">返回</v-btn>
            </v-toolbar-items>
        </v-toolbar>
        <v-container>
            <v-row>
                <v-col cols="auto"
                    ><v-img
                        :src="$store.state.currentSinger.headImgUrl"
                        max-height="200"
                        max-width="200"
                        class="rounded"
                    ></v-img
                ...
                <v-col>
                    <v-card dark flat>
                        ...
                        <v-card-subtitle>{{
                            $store.state.currentSinger.description
                        }}</v-card-subtitle>
                    </v-card>
                </v-col>
            </v-row>
                ...
                <v-col cols="12">
                    <v-list>
                        <v-list-item
                            ...
                            :key="item.name"
                            class="quaternary rounded mb-3"
                            dense
                            style="height: 50px"
                            link
                            @click="$yyHot.update('curPlayerIndex', index)"
                        >
